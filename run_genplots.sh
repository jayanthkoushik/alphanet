...
for i in 0 1 2; do
    ...
    for dataset in imagenetlt_resnext50_crt imagenetlt_resnext50_lws imagenetlt_resnext50_ride placeslt_resnet152_crt placeslt_resnet152_lws cifar100_resnet32_ltr cifar100_resnet32_ride; do
        ...
        (set -x; python run_makeplot.py PlotClsAccDeltaBySplit --base-res-dir "results/main/${dataset}" --exp-sub-dirs "rho_0.5" "rho_1" "rho_1.5" --exp-names "$\\rho=0.5$" "$\\rho=1$" "$\\rho=1.5$" --res-files-pattern "rep_${REP}/result.pth" --n-boot ${N_BOOT} --plot:width "full" --plot:aspect 1 --plot:theme "${theme}" --plot:context "${context}" --plot:font:default "${dfont}" --plot:font:math "${mfont}" --plot:file "${save_dir}/appendix/cls_deltas_${dname}${ext}";)
